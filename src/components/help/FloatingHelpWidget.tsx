...
export function FloatingHelpWidget() {
    ...
    useEffect(() => {
        ...
        setDefaultPosition({
            x: 28, // Half icon width to center on edge
            y: window.innerHeight - 28 // Bottom edge
        });
    }, []);
    ...
    useEffect(() => {
        const handleToggle = () => {
            ...
            if (saved !== null) {
                ...
                if (isVisible) {
                    // Reset to default position (Bottom Left) when restored
                    const defaultPos = {
                        x: 28, // Half icon width
                        y: window.innerHeight - 28 // Bottom edge
                    };
                    setPosition(defaultPos);
                    localStorage.setItem(HELP_POSITION_KEY, JSON.stringify(defaultPos));
                }
            }
        };
    ...
    // Handle drag end - save position without snapping
    const handleDragEnd = (_event: any, info: any) => {
        ...
        // Constrain X position within viewport (no snap)
        const constrainedX = Math.max(
            margin,
            Math.min(x, viewportWidth - iconSize - margin)
        );

        // Constrain Y position within viewport
        const constrainedY = Math.max(
            margin,
            Math.min(y, viewportHeight - iconSize - margin)
        );

        const newPos = { x: constrainedX, y: constrainedY };
        localStorage.setItem(HELP_POSITION_KEY, JSON.stringify(newPos));
        setPosition(newPos);
    };
